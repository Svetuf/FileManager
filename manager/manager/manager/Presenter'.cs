...
namespace manager
{
    class Presenter
    {
        ...
        private void openFile(object sender, EventArgs e)
        {
            if (view.getsetListView.SelectedItems[0].Tag.ToString() == "file")
            {
                if((view.getsetFi + "\\" + view.getsetListView.SelectedItems[0].Text).Contains(".zip"))
                {
                    ...
                    foldrrrr = new ZipFolder(view.getsetFi + "\\" + view.getsetListView.SelectedItems[0].Text);
                ...
                try
                {
                    if ((view.getsetFi + "\\" + view.getsetListView.SelectedItems[0].Text).Contains(".zip\\"))
                    {
                        ZipFile.OpenZipFile(view.getsetFi, view.getsetListView.SelectedItems[0].Text);
                    }
                }
                catch(Exception wer) { }
        ...
        protected void OnkoZakrito(string s, string dela)
        {
            if (dela == "copy")
            {
                try
                {
                    ...
                    {
                        FolderMethods.CopyDir(FileMethods.Combine(view.getsetFi, view.getsetListView.SelectedItems[0].Text), FileMethods.Combine(s, view.getsetListView.SelectedItems[0].Text));
                    }
                }
                catch (Exception e)
                {
        ...
        private void MENU2(object sender, EventArgs e)
        {
            ...
            {
                new ZipFile(view.getsetFi).Delete(view.getsetListView.SelectedItems[0].Text);
            }
            else
            {
        ...
        private void updateTheMenu(object sender, EventArgs e)
        {
            ...
            try
            {
                if(view.getsetFi.Contains(".zip"))
                {
                    ...
                    foldrrrr = new ZipFolder(view.getsetFi);
                    List<string> asss = new List<string>();
                    try
                    {
